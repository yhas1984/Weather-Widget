...
from PyQt6.QtGui import QIcon, QPainter, QColor, QPen, QFont, QLinearGradient, QBrush, QPolygonF, QRadialGradient
import math
import random
...
class WeatherWidget(QMainWindow):
    def __init__(self):
        ...
        self.cleanup_legacy_desktop_entries()
    ...
    @classmethod
    def config_path(cls):
        return cls.config_home() / "weather_widget_config.json"

    @classmethod
    def cleanup_legacy_desktop_entries(cls):
        """Remove launchers generated by old builds, without touching user-created entries."""
        config_home = cls.config_home()
        legacy_autostart = config_home / "autostart" / "WeatherWidget.desktop"
        try:
            if legacy_autostart.is_file():
                content = legacy_autostart.read_text(encoding="utf-8")
                generated_markers = (
                    "Name=WeatherWidget",
                    "Comment=Desktop Weather Widget",
                    "X-GNOME-Autostart-enabled=true",
                )
                if all(marker in content for marker in generated_markers):
                    legacy_autostart.unlink()
                    print(f"🧹 Entrada de inicio antigua eliminada: {legacy_autostart}")
        except OSError as exc:
            print(f"⚠️ No se pudo retirar el inicio antiguo: {exc}")

        launchpad_layout = config_home / "deepin" / "dde-launchpad" / "item-arrangement.ini"
        temp_name = None
        try:
            if not launchpad_layout.is_file():
                return
            content = launchpad_layout.read_text(encoding="utf-8")
            updated = content
            for token in (
                ", WeatherWidget.desktop", ",WeatherWidget.desktop",
                "WeatherWidget.desktop, ", "WeatherWidget.desktop,",
            ):
            ...
            with NamedTemporaryFile(
                "w", encoding="utf-8", dir=launchpad_layout.parent,
                prefix=f".{launchpad_layout.name}.", delete=False,
            ) as handle:
                handle.write(updated)
                handle.flush()
                os.fsync(handle.fileno())
                temp_name = handle.name
            os.replace(temp_name, launchpad_layout)
            temp_name = None
            print("🧹 Acceso antiguo retirado del lanzador de Deepin")
        except OSError as exc:
            print(f"⚠️ No se pudo actualizar el lanzador de Deepin: {exc}")
        finally:
            if temp_name:
                try:
...
if __name__ == "__main__":
    ...
    app.setDesktopFileName("weather-widget")
    icon_path = application_icon_path()
    if icon_path.is_file():
